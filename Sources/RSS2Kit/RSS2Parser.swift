...
///
/// `RSS2Parser` is the entry point for parsing RSS 2.0 feeds.
///
/// ## Example
///
/// ```swift
...
/// ## Thread Safety
///
/// `RSS2Parser` is `Sendable` and can be safely used from any actor or thread.
public struct RSS2Parser: Sendable {
    private let xmlParser: XMLDocumentParser
    private let channelParser: ChannelParser

    /// Creates a new RSS 2.0 parser.
    public init() {
        xmlParser = XMLDocumentParser()
        channelParser = ChannelParser()
    }

    /// Parses RSS 2.0 feed data.
    ///
    ...
    /// Parses an RSS 2.0 feed from a string.
    ///
